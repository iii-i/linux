...
def list_progs():
    ...
    for _, slot in radixtree.for_each_slot(idr_rt):
        progs.append(slot.dereference().cast(bpf_prog_ptr_type))
    return progs
...
def generate_debug_obj(ksym, prog):
    ...
    start = int(ksym["start"])
    code = gdb.selected_inferior().read_memory(start, int(ksym["end"]) - start)
