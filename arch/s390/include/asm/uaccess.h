...
__put_user_##type##_noinstr(unsigned type __user *to, unsigned type *from,\
			    unsigned long size)				\
{									\
...
{									\
	int err;							\
									\
	err = __put_user_##type##_noinstr(to, from, size);		\
...
static GET_PUT_USER_NOINSTR_ATTRIBUTES int				\
__get_user_##type##_noinstr(unsigned type *to, unsigned type __user *from,\
			    unsigned long size)				\
{									\
	union oac __oac_spec = {					\
...
{									\
	int err;							\
									\
	err = __get_user_##type##_noinstr(to, from, size);		\
